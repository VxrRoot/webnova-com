const UseValidation = (field: string, value: number | string) => {
    const regex = {
    ...
    switch (field) {
        case "name":
            if (!regex.name.test(String(value))) {
                return error_message("Nieprawidłowe imię");
            }
            break;
        case "mail":
            if (!regex.mail.test(String(value))) {
                return error_message("Nieprawidłowy adres e-mail");
            }
            break;
        case "message":
            if (!regex.message.test(String(value))) {
                return error_message(
                    "Wiadomość musi zawierać przynajmniej 4 znaki"
                );
            }
        default:
            return;
